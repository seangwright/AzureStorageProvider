...
namespace AzureStorageProvider
{
    ...
    {
        private Blob Get(string path) => BlobCollection.Instance.GetOrCreate(AzurePathHelper.GetBlobPath(path));
        ...
        public override void Copy(string sourceFileName, string destFileName, bool overwrite) => Get(sourceFileName).Copy(AzurePathHelper.GetBlobPath(destFileName), overwrite);
        ...
        /// </summary>
        /// <param name="path">Path to file.</param>
        public override byte[] ReadAllBytes(string path) => Get(path).Get();
        ...
        /// </summary>
        /// <param name="path">Path to file.</param> 
        public override CMS.IO.FileStream Create(string path) => new FileStream(path, FileMode.Create);
        ...
        public override void Move(string sourceFileName, string destFileName) => Get(sourceFileName).Move(AzurePathHelper.GetBlobPath(destFileName));
        ...
        /// </summary>
        /// <param name="path">Path to file.</param> 
        public override string ReadAllText(string path) => ReadAllText(path, EncodingHelper.DefaultEncoding);
        ...
        public override void WriteAllText(string path, string contents) => WriteAllText(path, contents, EncodingHelper.DefaultEncoding);
        ...
        /// <param name="contents">Content to write.</param>
        public override void AppendAllText(string path, string contents) => AppendAllText(path, contents, EncodingHelper.DefaultEncoding);
        ...
        public override CMS.IO.FileStream Open(string path, FileMode mode, FileAccess access) => new FileStream(path, mode, access);
        ...
        public override void SetLastWriteTimeUtc(string path, DateTime lastWriteTimeUtc) => SetLastWriteTime(path, lastWriteTimeUtc);
        ...
        /// </summary>
        /// <param name="path">Path to file.</param>
        public override FileSecurity GetAccessControl(string path) => new FileSecurity();
        ...
        /// </summary>
        /// <param name="path">Path to file.</param>
        public override DateTime GetLastWriteTime(string path) => Get(path).GetAttribute(a => a.LastModified);

        public DateTime GetCreationTime(string path)
        {
            string created = Get(path).GetMetadataAttribute(BlobMetadataEnum.DateCreated);
            if (!string.IsNullOrEmpty(created) && DateTime.TryParse(created, out var createdDate))
            {
                return createdDate;
            }
        ...
        public FileAttributes GetFileAttributes(string path) => (FileAttributes)Convert.ToInt32(Get(path).GetMetadataAttribute(BlobMetadataEnum.Attribute));

        public long GetLength(string path) => Get(path).GetAttribute(a => a.Length);
